...
export const OptimizedSearchBar: React.FC<OptimizedSearchBarProps> = ({ className }) => {
  const { 
    preschools, 
    searchFilters, 
    setSearchFilters, 
  ...
  const uniqueKommuner = useMemo(() => 
    [...new Set(preschools.map(p => p.kommun))].filter(Boolean).sort(),
    [preschools]
  );
  ...
  useEffect(() => {
    if (debouncedSearchQuery.length > 0) {
      const kommun = uniqueKommuner.find(k => 
        k.toLowerCase().includes(debouncedSearchQuery.toLowerCase())
      );
      
      if (kommun) {
        setSearchFilters({ kommuner: [kommun] });
      } else {
        setSearchFilters({ query: debouncedSearchQuery });
      }
          ...
          <div className="p-3">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span className="text-sm font-medium">Sök förskolor</span>
                {hasActiveFilters && (
