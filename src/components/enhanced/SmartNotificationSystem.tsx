...
export const SmartNotificationSystem: React.FC = () => {
  ...
  useEffect(() => {
    if (filteredPreschools.length > 0) {
      const highQualityCount = filteredPreschools.filter(p => 
        p.google_rating && p.google_rating >= 4.5 && 
        p.andel_med_förskollärarexamen && p.andel_med_förskollärarexamen >= 80
      ).length;
  ...
  if (!isVisible || notifications.length === 0) {
    return (
      <Button
        ...
        variant="outline"
        size="sm"
        className="fixed top-4 right-20 z-50 bg-card/95 backdrop-blur-sm border-border/50"
      >
